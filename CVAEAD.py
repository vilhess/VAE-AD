...
for anormal in range(10):
    ...
    with torch.no_grad():
        for i in range(10):
            inputs = test_dic_dataset[i].to(DEVICE)
    ...
    inputs_val = normal_val.to(DEVICE)
    with torch.no_grad():
        val_reconstructed, _, _ = model(inputs_val)

    val_scores = -torch.sum(((inputs_val - val_reconstructed)**2).flatten(start_dim=1), dim=1)
